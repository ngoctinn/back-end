...
def create_walk_in_customer(
	...
	**kwargs,
	...
	existing = crud.get_customer_by_phone_number(db, normalized_phone, include_deleted=False)
	if existing:
		raise PhoneNumberAlreadyExistsError(f"Số điện thoại {normalized_phone} đã tồn tại")
...
def verify_otp_and_link_account(
	...
	try:
		...
		stub_customer.deleted_at = datetime.utcnow()
		stub_customer.updated_at = datetime.utcnow()
...
def delete_customer(db: Session, customer_id: int) -> bool:
	...
	if not success:
		raise CustomerNotFoundError(f"Khách hàng {customer_id} không tìm thấy hoặc đã bị xóa")
	return success
